...
public class ReflectTools {
	...
	protected static final Object parser(String s, Class<?> clazz) {
		if (isBaseType(clazz)) {
	...
	protected static final boolean isBaseType(Class<?> clazz) {
		if (clazz == null) {
			return false;
		}
	...
	protected static final boolean isDefaultValue(Object value) {
		if (value == null) {
			return true;
	...
	protected static final boolean isString(Object object) {
		return String.class.isInstance(object);
	}

	protected static final boolean isNumber(Object object) {
		return Number.class.isInstance(object);
	}

	protected static final boolean isBoolean(Object object) {
		return Boolean.class.isInstance(object);
	}

	protected static final boolean isArray(Object object) {
	...
	protected static final boolean isArray(Class<?> clazz) {
		return clazz.isArray();
	}

	protected static final boolean isArray(Field field) {
		return isArray(field.getType());
	}

	protected static final Class<?> getClass(Object object) {
		if (object instanceof Class<?>) {
			return (Class<?>) object;
		} else if (object instanceof Field) {
	...
	protected static final Class<?> getClass(Type type) {
	...
	private static final boolean checkLegal(Field field) {
		...
		return Modifier.isFinal(modefiers) == false && field.getName().charAt(0) != '$';
	}
	...
	public static final List<Field> getFields(Class<?> clazz, boolean isCheckFinalAndEnum) {
		List<Field> list = new ArrayList<Field>();
		while (clazz != null && clazz.equals(Object.class) == false) {
			Field[] fields = clazz.getDeclaredFields();
			for (Field field : fields) {
				if (isCheckFinalAndEnum) {
					if (checkLegal(field)) {
						addField(field, list);
					}
				} else {
	...
	protected static final boolean isSubClass(Field field, Class<?> superClass) {
		return isSubClass(field.getType(), superClass);
	}

	protected static final boolean isSubClass(Class<?> childClass, Class<?> superClass) {
